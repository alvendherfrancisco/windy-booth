...
export default function Dashboard() {
  ...
  return <div className="space-y-7"><section><p className="text-sm text-[#8B8D93]">Welcome back{user?.full_name ? `, ${user.full_name.split(" ")[0]}` : ""}.</p><div className="mt-1 flex items-center gap-3"><h1 className="font-heading text-3xl font-extrabold">Your little booth</h1><span className="rounded-full bg-[#EFF3F7] px-3 py-1 text-xs font-bold capitalize text-[#3E5670]">{plan}</span></div></section>
    ...
    <section><div className="mb-4 flex items-end justify-between"><div><h2 className="font-heading text-xl font-extrabold">My Orders</h2><p className="text-sm text-[#8B8D93]">Your latest print updates.</p></div><Link to="/orders" className="text-sm font-bold text-[#3E5670]">See all</Link></div>{order ? <Link to="/orders" className="flex items-center gap-4 rounded-[18px] border border-[#D8D9DC] bg-white p-4"><StripPreview template={{ name: "Studio Mono" }} photos={orderStrip?.photo_urls || []} className="max-w-[74px]"/><div><p className="font-bold">{order.bundle_type} Bundle</p><p className="mt-1 text-sm capitalize text-[#3E5670]">{order.fulfillment_status}</p><p className="mt-1 text-xs text-[#8B8D93]">{new Date(order.created_date).toLocaleDateString()}</p></div></Link> : <div className="rounded-[18px] border border-dashed border-[#AEB0B5] p-8 text-center text-sm text-[#8B8D93]"><Package className="mx-auto mb-3" size={20}/>No orders yet.</div>}</section>
  </div>;
}