...
const DOODLE_STRIP_URL = "https://media.base44.com/images/public/6a60bb3456cf14775962b360/f60ca7e3d_generated_image.png";
...
export default function Dashboard() {
  ...
  return <div className="space-y-7"><section><div className="mt-1 flex items-center gap-3"><h1 className="font-heading text-3xl font-extrabold">Your booth</h1><span className="rounded-full bg-[#fff3bf] px-3 py-1 text-xs font-bold capitalize text-[#e67700]">{plan}</span></div></section>
    <div className="grid gap-4 lg:grid-cols-[minmax(0,1.6fr)_minmax(280px,0.8fr)]"><Link to="/booth" className="relative isolate block overflow-hidden rounded-[18px] bg-[#f06595] p-6 text-white transition hover:bg-[#e64980]"><div className="pointer-events-none absolute inset-0 overflow-hidden" style={{ zIndex: -1 }} aria-hidden="true"><div className="doodle-marquee flex flex-col"><img src={DOODLE_STRIP_URL} alt="" draggable={false} className="block w-full select-none" /><img src={DOODLE_STRIP_URL} alt="" draggable={false} className="block w-full select-none" /></div></div><Camera size={25} /><h2 className="mt-8 font-heading text-2xl font-extrabold">Start a new booth</h2><p className="mt-1 text-sm text-white/85">Pick a frame, strike a pose, take it with you.</p><span className="mt-5 inline-flex items-center gap-2 text-sm font-bold">Begin <ArrowRight size={16} /></span></Link>
