...
export default function Dashboard() {
  ...
  const load = async () => {
    if (!user) return;
    const [recentStrips, orders] = await Promise.all([
      base44.entities.Strip.filter({ user_id: user.id, saved: true }, "-created_at", 5),
      base44.entities.Order.filter({ user_id: user.id }, "-created_date", 1),
    ]);
    setStrips(recentStrips);
    setOrder(orders[0]);
