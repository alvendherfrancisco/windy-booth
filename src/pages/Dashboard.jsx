...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    if (p.get("unlock") === "success" && sid) {
      base44.functions.invoke("confirmUnlock", { session_id: sid }).then((r) => {
        const d = r?.data ?? r;
        if (d?.type === "lifetime") updateUser({ plan: "lifetime" });
        else if (d?.type === "collection" && d.category) updateUser({ owned_collections: [...(user?.owned_collections || []), d.category] });
  ...
  return <div className="space-y-7"><section><div className="mt-1 flex items-center gap-3"><h1 className="font-heading text-3xl font-extrabold">Your booth</h1><span className="rounded-full bg-[#fff3bf] px-3 py-1 text-xs font-bold text-[#e67700]">{planLabel(user)}</span></div></section>
    <div className="grid gap-4 lg:grid-cols-[minmax(0,1.6fr)_minmax(280px,0.8fr)]"><Link to="/booth" className="relative isolate block overflow-hidden rounded-[18px] bg-[#f06595] p-6 text-white transition hover:bg-[#e64980]"><HeroFaceScatter /><Camera size={25} className="hidden" /><h2 className="mt-8 font-heading text-2xl font-extrabold">Start a new booth</h2><p className="mt-1 text-sm text-white/85">Pick a frame, strike a pose, take it with you.</p><span className="mt-5 inline-flex items-center gap-2 text-sm font-bold">Begin <ArrowRight size={16} /></span></Link>
    ...
    {!lifetime && <section className="relative isolate overflow-hidden rounded-[18px] bg-gradient-to-r from-[#f06595] to-[#cc5de8] p-5 text-white"><Sparkles size={20} className="text-white/90" /><p className="mt-2 font-heading text-lg font-extrabold">Create without limits</p><p className="mt-1 text-sm text-white/85">Unlock unlimited booth sessions and every exclusive artist-designed collection.</p><button onClick={() => setUpgradeOpen(true)} className="mt-3 inline-flex items-center gap-1.5 rounded-full bg-white px-4 py-2 text-sm font-bold text-[#e64980] transition hover:bg-white/90">Get Lifetime Pass <ArrowRight size={15} /></button></section>}
    ...
    {printShopEnabled && <section><div className="mb-4 flex items-end justify-between"><div><h2 className="font-heading text-xl font-extrabold">My Orders</h2><p className="text-sm text-[#8B8D93]">Your latest print updates.</p></div><Link to="/orders" className="text-sm font-bold text-[#228be6]">See all</Link></div>{order ? <Link to="/orders" className="flex items-center gap-4 rounded-[18px] border border-[#D8D9DC] bg-white p-4"><StripPreview template={templates[orderStrip?.template_id] || { name: "Studio Mono" }} photos={orderStrip?.photo_urls || []} className="max-w-[74px]" /><div><p className="font-bold">{order.bundle_type} Bundle</p><p className="mt-1 inline-block rounded-full bg-[#d0ebff] px-2.5 py-0.5 text-xs font-bold capitalize text-[#228be6]">{order.fulfillment_status}</p><p className="mt-1 text-xs text-[#8B8D93]">{new Date(order.created_date).toLocaleDateString()}</p></div></Link> : <div className="rounded-[18px] border border-dashed border-[#AEB0B5] p-8 text-center text-sm text-[#8B8D93]"><Package className="mx-auto mb-3" size={20} />No orders yet.</div>}</section>}
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
        <div className="relative w-full max-w-sm overflow-hidden rounded-[22px] border border-[#E8E2D8] bg-[#ebfbee] p-6 text-center animate-modal-in">
          <div className="mx-auto flex h-12 w-12 items-center justify-center rounded-full bg-white text-[#37b24d]"><Check size={26} /></div>
          <h2 className="mt-3 font-heading text-2xl font-extrabold text-[#2D2D2D]">{unlockResult?.type === "lifetime" ? "Welcome to Lifetime Pass!" : "Collection unlocked!"}</h2>
          <p className="mt-2 text-sm text-[#5C5953]">{unlockResult?.type === "lifetime" ? "You now have unlimited booth sessions, every artist-designed collection, and unlimited saved strips." : `${unlockResult?.category} is now yours to use.`}</p>
          <button onClick={() => setUnlockResult(null)} className="mt-5 rounded-full bg-[#f06595] px-5 py-3 text-sm font-bold text-white transition hover:bg-[#e64980]">Enjoy making memories</button>
        </div>
      </div>
      )}
