...
export default function AppShell() {
  ...
  return <div className="min-h-screen bg-[#FAF9F6] text-[#15161A] pb-20 md:pb-0 md:pl-60">
    <aside className="hidden md:fixed md:inset-y-0 md:left-0 md:flex md:w-60 md:flex-col md:border-r md:border-[#D8D9DC] md:bg-[#F1F0EC] md:p-6">
      <Link to="/dashboard" className="flex items-center gap-2 font-heading text-2xl font-extrabold tracking-tight"><Flower2 size={23} strokeWidth={1.7} className="text-[#8AA3BE]"/>Vendi</Link><nav className="mt-12 space-y-2">{items.map(({to,label,icon:Icon}) => <Link key={to} to={to} className={`flex items-center gap-3 rounded-xl px-4 py-3 text-sm font-bold ${location.pathname === to ? "bg-[#15161A] text-white" : "text-[#55575E] hover:bg-white"}`}><Icon size={19}/>{label}</Link>)}</nav>
    </aside>
