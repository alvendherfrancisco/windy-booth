...
export default function AppShell() {
  ...
  return <div className="min-h-screen bg-white text-[#15161A] pb-20 md:pb-0 md:pl-60">
    <aside className="hidden md:fixed md:inset-y-0 md:left-0 md:flex md:w-60 md:flex-col md:border-r md:border-[#D8D9DC] md:bg-[#F1F0EC] md:p-6">
      <Link to="/dashboard" className="flex items-center gap-2 font-heading text-2xl font-extrabold tracking-tight"><Flower2 size={23} strokeWidth={1.7} className="text-[hsl(var(--foreground))]" />Vendi</Link><nav className="mt-12 space-y-2">{items.map(({ to, label, icon: Icon }) => <Link key={to} to={to} className={`flex items-center gap-3 rounded-xl px-4 py-3 text-sm font-bold ${location.pathname === to ? "bg-[#15161A] text-white" : "text-[#55575E] hover:bg-white"}`}><Icon size={19} />{label}</Link>)}</nav>
    </aside>
    <header className="sticky top-0 z-20 flex h-16 items-center justify-end border-b border-[#D8D9DC] bg-white/95 px-5 backdrop-blur md:px-10"><Link to="/notifications" className="relative rounded-full p-2 hover:bg-[#EFF3F7]"><Bell size={21} />{unread > 0 && <span className="absolute right-1 top-1 flex h-4 min-w-4 items-center justify-center rounded-full bg-[#8AA3BE] px-1 text-[9px] font-bold text-white">{unread}</span>}</Link></header>
    <main className="mx-auto max-w-5xl px-4 py-6 md:px-10 md:py-10"><Outlet /></main>
    <nav className="fixed inset-x-0 bottom-0 z-30 flex border-t border-[#D8D9DC] bg-white px-2 pb-[max(0.5rem,env(safe-area-inset-bottom))] pt-2 md:hidden">{items.map(({ to, label, icon: Icon }) => <Link key={to} to={to} className={`flex flex-1 flex-col items-center gap-1 py-1 text-[10px] font-bold ${location.pathname === to ? "text-[#3E5670]" : "text-[#8B8D93]"}`}><Icon size={19} />{label}</Link>)}</nav>
  </div>;
}