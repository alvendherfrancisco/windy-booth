...
export default function AppShell() {
  ...
  return <div className="min-h-screen bg-[#FAF9F6] text-[#15161A] pb-20 md:pb-0 md:pl-60">
    ...
    <header className="sticky top-0 z-20 flex h-16 items-center justify-between border-b border-[#D8D9DC] bg-[#FAF9F6]/95 px-5 backdrop-blur md:px-10"><Link to="/dashboard" className="flex items-center gap-2 font-heading text-xl font-extrabold tracking-tight md:hidden"><Flower2 size={20} strokeWidth={1.7} className="text-[#8AA3BE]" />Vendi</Link><span className="hidden text-sm text-[#8B8D93] md:block">Make a little memory.</span><Link to="/notifications" className="relative rounded-full p-2 hover:bg-[#EFF3F7]"><Bell size={21} />{unread > 0 && <span className="absolute right-1 top-1 flex h-4 min-w-4 items-center justify-center rounded-full bg-[#8AA3BE] px-1 text-[9px] font-bold text-white">{unread}</span>}</Link></header>
