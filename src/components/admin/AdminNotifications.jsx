...
export default function AdminNotifications({ users, onChanged }) {
  ...
  return (
    <div className="space-y-4">
      <div>
        <h2 className="font-heading text-xl font-extrabold">Broadcast <span className="text-sm font-medium text-[#8A8580]">to {users.length} users</span></h2>
        <p className="text-sm text-[#8A8580]">Send an in-app notification to every registered user.</p>
      </div>

      <div className="rounded-2xl border border-[#E8E2D8] bg-white p-5">
        <div className="grid gap-3 sm:grid-cols-2">
          <label className="block">
            <span className="mb-1 block text-xs font-bold uppercase tracking-wider text-[#8A8580]">Type</span>
            <select value={type} onChange={(e) => setType(e.target.value)} className="input">
              {TYPES.map((t) => <option key={t} value={t} className="capitalize">{t.replace("_", " ")}</option>)}
            </select>
          </label>
          <label className="block">
            <span className="mb-1 block text-xs font-bold uppercase tracking-wider text-[#8A8580]">Link (optional)</span>
            <input value={link} onChange={(e) => setLink(e.target.value)} className="input" placeholder="/print-shop" />
          </label>
        </div>
        <label className="mt-3 block">
          <span className="mb-1 block text-xs font-bold uppercase tracking-wider text-[#8A8580]">Message</span>
        ...
        <div className="mt-3 flex items-center justify-between">
          <span className="flex items-center gap-1.5 text-xs text-[#8A8580]"><Bell size={13} /> Delivered instantly to all inboxes</span>
          <button onClick={send} disabled={busy || !message.trim()} className="inline-flex items-center gap-1.5 rounded-full bg-[#e64980] px-5 py-2.5 text-sm font-bold text-white hover:bg-[#d04072] disabled:bg-[#E8E2D8] disabled:text-[#8A8580]">
